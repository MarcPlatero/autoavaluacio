...
The project was built using Laravel, along with Vue.js, Blade, and PHP. The system works well overall, although there are still some features that could be added (it wasn’t required to include every possible feature — only the most essential ones). The design wasn’t a priority — it’s quite basic and was mainly built to demonstrate functionality. Some components, like “Modules”, are not yet implemented and are currently displayed in the same way as “Cycles”, just to showcase the basic structure and functionality.
